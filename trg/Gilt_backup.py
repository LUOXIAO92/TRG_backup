...
def Gilt_plaq_2dHOTRG(T0:xp.ndarray, T1:xp.ndarray, gilt_eps, leg:str, direction:str):
    ...
    #from trg.__gilts__ import Ropt
    #from abeliantensors import Tensor
    #U = Tensor.from_ndarray(U.get())
    #S = Tensor.from_ndarray(S.get())
    #Rprime, count = Ropt(U, S, gilt_eps)
    #Rprime = xp.asarray(Rprime.to_ndarray())
    #U = xp.asarray(U.to_ndarray())
    #S = xp.asarray(S.to_ndarray())
    #Rp_ai, sRp, Rp_ib = svd(Rprime, shape=[[0],[1]], truncate_err=gilt_eps*1e-3, split=True)
    #print("sRp:", sRp)
    #err = gilt_error(U, S, Rp_ai, Rp_ib)
    #done = xp.abs(sRp-1).max() < convergence_err
...
def gilt_plaq_2dHOTRG_22222(T0, T1, gilt_eps, direction):
